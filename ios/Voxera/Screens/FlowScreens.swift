...
enum ResultHints {
  static func psytypeBody(typeKey: String, language: AppLanguage) -> String {
    ...
    switch language {
    case .ru: return ru[key] ?? "Краткое описание этого психотипа пока недоступно."
    case .en, .zh, .kz: return en[key] ?? "A short description for this psychotype is not available yet."
    }
  }
  ...
  static func emoMetricBody(scaleKey: String, language: AppLanguage, fallback: String) -> String {
    ...
    switch language {
    case .ru: return ru[key] ?? fallback
    case .en, .zh, .kz: return en[key] ?? fallback
    }
  }
